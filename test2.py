...
for itr in range(emitr):
    ...
    for e in range(n_env): 
        ...
        plot_intensity = 1
        if plot_intensity: 
            sns.heatmap(wblval(T, n_cty, env_data[e]["alpha"], 0, env_data[e]["beta"]))
            plt.title(f'Estimated Weibull distribution at itr:{itr} for env{e+1}')
            plt.show()
            sns.heatmap(env_data[e]["p_c_ij"])
            plt.title(f'p_c_ij at itr:{itr} for env{e+1}')
            plt.show()
            
            plt.figure(figsize=(10,6))
            sns.heatmap(env_data[e]["lambda_t"].reshape(n_cty, n_day))
            plt.title(f'Conditional intensity at itr:{itr} for env{e+1}')
            plt.xlabel('Days')
            plt.ylabel('Counties')
            #plt.savefig(os.path.join(checkpoint_dir, f'intensity_heatmap_itr{itr}_env{e+1}.png'))
            plt.show()
            #plt.close()
    ...
    for e in range(n_env):
        ...
        plot_weibull = 1
        if plot_weibull and len(obs) > 0:
            
            plt.figure(figsize=(10, 6))
            plt.hist(sampled_times, bins=50, density=True,
                    alpha=0.7, label='Observed intervals')
            
            # Fit line
            x = np.linspace(0, max(sampled_times), 100)
            if env_data[e]["alpha"] > 0 and env_data[e]["beta"] > 0:  # Only plot if parameters are valid
                pdf = weibull_min.pdf(x, env_data[e]["beta"], loc=0, scale=env_data[e]["alpha"])
                plt.plot(x, pdf, 'r-', lw=2, label=f'Weibull fit (α={env_data[e]["alpha"]:.2f}, β={env_data[e]["beta"]:.2f})')
            
            plt.title(f'Inter-event Time Distribution - Environment {e+1}, Iteration {itr+1}')
            plt.xlabel('Inter-event Time')
            plt.ylabel('Density')
            plt.legend()
            plt.show()
            #plt.savefig(os.path.join(checkpoint_dir, f'weibull_dist_env{e+1}_itr{itr+1}.png'))
            #plt.close()
